...
import {battleImgAddress, traderImgAddress} from "@/assets/img/returnImgByName";
...
//取得分类后的职业二维数组
export const getDetailsCategoryCareer = (initializeArray: Array<any>) => {
    //排序
...
export const CategoryCareer: React.FC<Props> = (props) => {
    ...
    const renderCareerCategory = useCallback((careerList: Array<any>, selectedList: Array<any>) => {
        ...
        return careerList && getDetailsCategoryCareer(careerList)?.map((item: any, index) => {
            return <div className="introduction-details-career-first-category" key={index}>
                {item.map((item2: any) => {
                    const {id, name} = item2
                    return <div className={
                        classnames("introduction-details-career-details-item", {
